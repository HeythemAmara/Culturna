...
class UtilisateurC
{
    ...
    public function BanUtilisateur(int $id)
    {
        try {

            $pdo = config::getConnexion();
            $sql = "UPDATE utilisateurs SET Perm = 'ban' WHERE IdU=" . $id . "";
            $query = $pdo->prepare($sql);
            $query->execute();
            echo $query->rowCount() . " records deleted";
        } catch (PDOException $e) {
...
public function Username($id) {
    try {
        $pdo = Config::getConnexion();
        $sql = "SELECT Username FROM utilisateurs WHERE IdU = :id";
        $query = $pdo->prepare($sql);
        $query->execute(['id' => $id]);
        $result = $query->fetch();
        return $result['Username'] ?? null; // Use the null coalescing operator to avoid errors if the result is empty
    } catch (PDOException $e) {
...
public function login($Username, $Mdp)
{
        try {
            $pdo = Config::getConnexion();
            $sqll = "SELECT Mdp FROM utilisateurs WHERE Username = :Username";
            $query = $pdo->prepare($sqll);
            $query->execute(['Username' => $Username]);
            $result = $query->fetch();
            $password=$result['Mdp'] ?? null; // Use the null coalescing operator to avoid errors if the result is empty
        } catch (PDOException $e) {
            // Log the error or throw an exception
            error_log("Error fetching Mdp for username $Username: " . $e->getMessage());
            throw $e;
        }
        if ($password == $Mdp) 
        {
        ...
        { // Add variable name for nested catch block
            echo "error add: " . $ex->getMessage();
            return 0;
        }
    }
