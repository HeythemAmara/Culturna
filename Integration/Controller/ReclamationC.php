...
class ReclamationC
{
    ...
    public function updateReclamationstatus( $id)
    {
        try {
            ...
            $pdo = config::getConnexion();
            $sql = "UPDATE `reclamation` SET `Statut_R` = 'resolut' WHERE `id_R` =:idrr";
            $query = $pdo->prepare($sql);
            $query->execute([
    ...
    public function emailreclamation($id)
    {
        try 
        {
            $pdo = config::getConnexion();
                $sql = "SELECT * FROM `reclamation` WHERE id_R=:idrr ORDER BY id_R ASC LIMIT 1";
                $query = $pdo->prepare($sql);
                $query->execute([
