...
function deleteExpiredEvents()
{
    ...
    try {
        $pdo = Config::getConnexion();
        $sql = "SELECT nbrPlaceMax FROM event WHERE idEvent = :id";
        $query = $pdo->prepare($sql);
        $query->execute(['id' => $id]);
        $result = $query->fetch();
        return $result['nbrPlaceMax'] ?? null; // Use the null coalescing operator to avoid errors if the result is empty
    } catch (PDOException $e) {
        // Log the error or throw an exception
        error_log("Error fetching nbrPlaceMax for ID $id: " . $e->getMessage());
        throw $e;
    }
}
