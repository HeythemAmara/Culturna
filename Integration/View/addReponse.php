...
$ReclamationC2 = new ReclamationC();
...
$SRadd = $_POST['SRadd'];
$MRadd = $_POST['MRadd'];
$idRecadd = $_POST['idRecadd'];
$idClientadd = $_POST['idClientadd'];
if (
    isset($_POST['SRadd']) &&
    isset($_POST['MRadd']) &&
    isset($_POST['idRecadd']) &&
    isset($_POST['idClientadd']) 
   ) 
{
    if (
      //  !empty($_POST['idRepadd']) &&
        !empty($_POST['SRadd']) &&
        !empty($_POST['MRadd']) &&
        !empty($_POST['idRecadd']) &&
        ...
        $ReclamationC->updateReclamationstatus($_POST['idRecadd']);
...
if(isset($_POST['Add'])) {       
    $list = $ReclamationC2->emailreclamation($_POST['idRecadd']);

    foreach ($list as $Userr){} ;  
        // Configuration de PHPMailer
        $mail = new PHPMailer(true);
 
        $email = $Userr['Email'];      //receiver Email
        $messagebody    = 'Sujet de la réponse : '.$SRadd.'<br>Message de la réponse : '.$MRadd.'<br>ID de la réclamation : '.$idRecadd.'<br>ID du client : '.$idClientadd;
            // Envoi du mail de rappel
            try {
                $mail->isSMTP();
                $mail->Host= 'smtp.gmail.com';
                $mail->SMTPAuth= true;
                $mail->Username= 'culturnaskapere111111@gmail.com';
                $mail->Password= 'gmmxjowkagiwjibw
                ';
                $mail->SMTPSecure= PHPMailer::ENCRYPTION_STARTTLS;
                $mail->Port= 587;
    
                $mail->setFrom('culturnaskapere111111@gmail.com', 'culturna'); 
